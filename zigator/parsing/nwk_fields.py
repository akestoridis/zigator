...
def nwk_routerequest(pkt):
    ...
    if not config.set_entry(
            ...
            MTO_STATES):
        config.entry["error_msg"] = "Unknown MTO state"
        return
    if not config.set_entry(
            "nwk_routerequest_ed",
            pkt[ZigbeeNWKCommandPayload].dest_addr_bit,
            ED_STATES):
        config.entry["error_msg"] = "Unknown ED state"
        return
    if not config.set_entry(
            "nwk_routerequest_mc",
            pkt[ZigbeeNWKCommandPayload].multicast,
            MD_STATES):
        config.entry["error_msg"] = "Unknown MD state"
        return
...
def nwk_routereply(pkt):
    ...
    if not config.set_entry(
            ...
            EO_STATES):
        config.entry["error_msg"] = "Unknown EO state"
        return
    if not config.set_entry(
            "nwk_routereply_er",
            pkt[ZigbeeNWKCommandPayload].responder_addr_bit,
            ER_STATES):
        config.entry["error_msg"] = "Unknown ER state"
        return
    if not config.set_entry(
            "nwk_routereply_mc",
            pkt[ZigbeeNWKCommandPayload].multicast,
            MR_STATES):
        config.entry["error_msg"] = "Unknown MR state"
        return
...
def nwk_networkstatus(pkt):
    ...
    if not config.set_entry(
            ...
            STATUS_CODES):
        config.entry["error_msg"] = "Unknown status code"
        return
...
def nwk_leave(pkt):
    ...
    if not config.set_entry(
            ...
            REJOIN_STATES):
        config.entry["error_msg"] = "Unknown rejoin state"
        return
    if not config.set_entry(
            "nwk_leave_request",
            pkt[ZigbeeNWKCommandPayload].request,
            REQUEST_STATES):
        config.entry["error_msg"] = "Unknown request state"
        return
    if not config.set_entry(
            "nwk_leave_rmch",
            pkt[ZigbeeNWKCommandPayload].remove_children,
            RC_STATES):
        config.entry["error_msg"] = "Unknown RC state"
        return
...
def nwk_rejoinrsp(pkt):
    ...
    if not config.set_entry(
            ...
            REJOIN_STATUSES):
        config.entry["error_msg"] = "Unknown rejoin status"
        return
...
def nwk_linkstatus(pkt, msg_queue):
    ...
    if not config.set_entry(
            ...
            FIRST_FRAME_STATUSES):
        config.entry["error_msg"] = "Unknown first frame status"
        return
    if not config.set_entry(
            "nwk_linkstatus_last",
            pkt[ZigbeeNWKCommandPayload].last_frame,
            LAST_FRAME_STATUSES):
        config.entry["error_msg"] = "Unknown last frame status"
        return
...
def nwk_edtimeoutreq(pkt):
    ...
    if not config.set_entry(
            ...
            RT_VALUES):
        config.entry["error_msg"] = "Unknown RT value"
        return
...
def nwk_edtimeoutrsp(pkt):
    ...
    if not config.set_entry(
            ...
            RT_STATUSES):
        config.entry["error_msg"] = "Unknown RT status"
        return
    ...
    if not config.set_entry(
            ...
            POLL_STATES):
        config.entry["error_msg"] = "Unknown poll state"
        return
    if not config.set_entry(
            "nwk_edtimeoutrsp_timeout",
            pkt[ZigbeeNWKCommandPayload].ed_timeout_req_keepalive,
            TIMEOUT_STATES):
        config.entry["error_msg"] = "Unknown timeout state"
        return
...
def nwk_command(pkt, msg_queue):
    ...
    if (config.entry["nwk_extendeddst"]
        ...
        config.entry["error_msg"] = "Invalid Extended Destination state"
        return
    ...
    if (config.entry["nwk_extendedsrc"]
        ...
        config.entry["error_msg"] = "Invalid Extended Source state"
        return
    ...
    if config.entry["nwk_srcroute"] == "NWK Source Route Included":
        ...
        config.entry["error_msg"] = "Invalid Source Route state"
        return
...
def nwk_beacon(pkt):
    ...
    if not config.set_entry(
            ...
            NWK_PROTOCOL_VERSIONS):
        config.entry["error_msg"] = "Unknown NWK protocol version"
        return
    if not config.set_entry(
            "nwk_beacon_routercap",
            pkt[ZigBeeBeacon].router_capacity,
            ROUTER_CAPACITIES):
        config.entry["error_msg"] = "Unknown router capacity"
        return
    ...
    if not config.set_entry(
            ...
            END_DEVICE_CAPACITIES):
        config.entry["error_msg"] = "Unknown end device capacity"
        return
...
def nwk_auxiliary(pkt, msg_queue):
    ...
    if not config.set_entry(
            ...
            NWK_SECURITY_LEVELS):
        config.entry["error_msg"] = "Unknown NWK security level"
        return
    if not config.set_entry(
            "nwk_aux_keytype",
            pkt[ZigbeeSecurityHeader].key_type,
            NWK_KEY_TYPES):
        config.entry["error_msg"] = "Unknown NWK key type"
        return
    if not config.set_entry(
            "nwk_aux_extnonce",
...
def nwk_fields(pkt, msg_queue):
    ...
    if not config.set_entry(
            ...
            NWK_DR_STATES):
        config.entry["error_msg"] = "Unknown NWK DR state"
        return
